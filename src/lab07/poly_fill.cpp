...
struct App
{
    ...
    std::vector<Cell> filled_cells;  ///< Half-space fill result.
    bool polygon_is_convex =
        true;  ///< False when vertices >= 3 form a non-convex polygon.
...
constexpr float kVertexColor[3] = {1.0f, 0.2f, 0.2f};
...
/**
 * @brief Two-color palette for one geometric region.
 *
 * Each region owns exactly two edge colors:
 * `e_color` is used for pixels produced by an `E` step, and
 * `ne_color` is used for pixels produced by an `NE` step.
 *
 * Vertex cells do not use this palette; they are always rendered with the
 * global red `kVertexColor` so endpoints stay visually consistent.
 */
struct RegionStyle
{
    Color3 e_color;
...
/**
 * @brief Per-region E/NE color pairs for edge pixel rendering.
 *
 * The outer array index is `region - 1`, so `kRegionStyles[0]` corresponds to
 * Region 1 and `kRegionStyles[7]` corresponds to Region 8.
 *
 * Color selection is a two-stage lookup:
 * 1. The line segment's geometric region chooses one `RegionStyle`.
 * 2. The pixel's Bresenham step type (`E` or `NE`) chooses one of the two
 *    colors inside that style.
 *
 * This means edge pixels communicate two pieces of information at once:
 * region distinguishes the segment orientation, while `E` vs. `NE`
 * distinguishes whether the step advanced only on the major axis or on both
 * axes. Vertices are intentionally excluded from this scheme and stay red.
 */
constexpr std::array<RegionStyle, 8> kRegionStyles = {{
    RegionStyle{{0.20f, 0.85f, 0.20f}, {0.20f, 0.50f, 1.00f}},
    RegionStyle{{0.40f, 0.90f, 0.30f}, {0.10f, 0.80f, 1.00f}},
    RegionStyle{{0.65f, 0.85f, 0.25f}, {0.20f, 0.70f, 0.95f}},
    RegionStyle{{0.85f, 0.75f, 0.20f}, {0.30f, 0.65f, 1.00f}},
    RegionStyle{{0.90f, 0.55f, 0.20f}, {0.35f, 0.55f, 0.95f}},
    RegionStyle{{0.90f, 0.40f, 0.30f}, {0.45f, 0.50f, 0.95f}},
    RegionStyle{{0.80f, 0.35f, 0.55f}, {0.55f, 0.45f, 1.00f}},
    RegionStyle{{0.65f, 0.40f, 0.80f}, {0.65f, 0.55f, 1.00f}},
}};
 ...
 *
 * Unlike Lab05's toggle set, Lab06 maintains ordered vertices and a derived
 * edge cache, so both must be cleared when `N` changes.
 *
...
/**
 * @brief Draw the cached half-space fill cells for the current polygon.
 *
...
/**
 * @brief Draw all cached line pixels using the region-specific color mapping.
 *
 * For each edge, the renderer first selects `kRegionStyles[region - 1]`.
 * Then each pixel chooses either `e_color` or `ne_color` according to its
 * stored `StepType`. This keeps the color meaning stable across all regions:
 * one color represents "major-axis only" and the other represents
 * "major-axis plus minor-axis".
 *
...
void draw_line_pixels(const App& state) {
    ...
    for (const LineSegment& edge : state.edges) {
        assert(edge.region >= 1 && edge.region <= 8);
        const RegionStyle& style =
            kRegionStyles[static_cast<std::size_t>(edge.region - 1)];
        for (const RasterPixel& pixel : edge.pixels) {
            glColor3fv(
                (pixel.step == StepType::E ? style.e_color : style.ne_color)
                    .data());
 ...
 * when the polygon edge passes through the same cell. This also keeps vertices
 * visually separate from the region-based edge palette: all endpoints use the
 * same fixed red, regardless of which region the adjacent edges belong to.
 *
 * @param state Current polygon state.
 */
void draw_vertices(const App& state) {
    glColor3fv(kVertexColor);
    glBegin(GL_QUADS);
...
/**
 * @brief Test whether an ordered polygon is convex.
 *
 * Handles both CW and CCW input by normalising the sign of the cross products
 * against the polygon's own winding.  A degenerate (zero-area) polygon is
 * reported as non-convex so the fill pass is skipped.
 *
 * @param verts Ordered polygon vertices.
 * @return True if every interior angle is convex (cross product same-signed).
 */
bool is_convex(const std::vector<Cell>& verts) {
    const std::size_t n = verts.size();
    if (n < 3) return true;
    const int area2 = signed_area2(verts);
    if (area2 == 0) return false;
    const int sign = area2 > 0 ? 1 : -1;
    for (std::size_t i = 0; i < n; ++i) {
        const Cell& a = verts[i];
        const Cell& b = verts[(i + 1) % n];
        const Cell& c = verts[(i + 2) % n];
        const int cross = (b.x - a.x) * (c.y - b.y) - (b.y - a.y) * (c.x - b.x);
...
void rebuild_edges(App& state) {
    ...
    state.edges.reserve(n >= 3 ? n : 1);
    for (std::size_t i = 0; i < (n >= 3 ? n : 1); ++i) {
        const Cell& from = state.vertices[i];
        const Cell& to = state.vertices[(i + 1) % n];
        LineSegment edge = rasterize_line(from, to);
        edge.from_idx = static_cast<int>(i);
        edge.to_idx = static_cast<int>((i + 1) % n);
        if (edge.region != 0) {
    ...
    if (n >= 3) {
        state.polygon_is_convex = is_convex(state.vertices);
        if (state.polygon_is_convex) {
            state.filled_cells = fill_polygon_half_space(state.vertices);
        }
    }
...
/**
 * @brief Build the Lab07 overlay status string.
 *
...
/**
 * @brief Render the Lab07 scene in grid, fill, line-pixel, vertex order.
 *
 * The half-space interior is drawn first, then the Lab06 edge rasterization is
 * layered on top, and finally the clicked vertices remain visible above both.
 */
void App::display_cb() {
    App& state = app();
    const int width = glutGet(GLUT_WINDOW_WIDTH);
...
/**
 * @brief Handle Lab06 keyboard actions.
 *
...
int main(int argc, char** argv) {
    ...
    const int win = glutCreateWindow("Polygon Half-Space Fill");
    if (win <= 0) return 1;
