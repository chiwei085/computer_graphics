...
/**
 * @brief Build the Lab06 overlay status string.
 *
...
/**
 * @brief Render the Lab06 scene in grid, line-pixel, vertex, overlay order.
 *
 * This differs from Lab05 by drawing rasterized polygon edges instead of a
 * toggle-filled cell set, and by rendering vertices last to keep endpoints on
 * top.
 */
void App::display_cb() {
    App& state = app();
...
int main(int argc, char** argv) {
    ...
    const int win = glutCreateWindow("Polygon Line Rasterizer");
    if (win <= 0) return 1;
