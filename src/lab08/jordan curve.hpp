...
inline std::vector<Cell> fill_polygon_non_zero(
    ...
    int x_min = vertices[0].x, x_max = vertices[0].x;
    int y_min = vertices[0].y, y_max = vertices[0].y;
    for (const Cell& v : vertices) {
    ...
    result.reserve(static_cast<std::size_t>(x_max - x_min + 1) *
                   static_cast<std::size_t>(y_max - y_min + 1));
